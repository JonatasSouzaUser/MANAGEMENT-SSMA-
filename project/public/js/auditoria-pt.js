...
const sPT= document.querySelector('#mPT');
...
function imprimirLancamentos() {
  // Construir uma representação da tabela em formato de string
  let tableHTML = "<table><thead><tr><th>Auditoria</th><th>Líder</th><th>Unidade</th><th>Local</th><th>Tipo de PT</th><th>Desvio</th><th>Descrição do desvio</th><th>Tratativa</th></thead><tbody>";
  
  // Iterar sobre os lançamentos e adicionar as linhas à representação da tabela
  itens.forEach(item => {
      tableHTML += `<tr><td>${item.auditoria}</td><td>${item.líder}</td><td>${item.unidade}</td><td>${item.local}</td><td>${item.especialidade}</td><td>${item.pt}</td><td>${item.desvio}</td><td>${item.descrição}</td><td>${item.tratativa}</td></tr>`;
  });
